...
class MnasNet(nn.Module):
    def __init__(self, n_class=1000, input_size=224, width_mult=1.):
        ...
        input_channel = _round_to_multiple_of(32 * width_mult, 8)
        self.last_channel = _round_to_multiple_of(1280 * width_mult, 8) #if width_mult > 1.0 else 1280
        output_channel = _round_to_multiple_of(16 * width_mult, 8)
        # building first two layer
        self.features = [Conv_3x3(1, input_channel, 2), SepConv_3x3(input_channel, output_channel)]
        input_channel = output_channel

        # building inverted residual blocks (MBConv)
        for t, c, n, s, k in self.interverted_residual_setting:
            output_channel = _round_to_multiple_of(c * width_mult, 8)
            for i in range(n):
                if i == 0:
